...
@Component({
  selector: 'app-cart',
  imports: [NoDataComponent,CommonModule, ReactiveFormsModule, RouterModule],
...
export class CartCommon {
  ...
  searchAddress(event: any) {
    ...
    this.http
      .get(
        `https://nominatim.openstreetmap.org/search?format=json&addressdetails=1&countrycodes=in&q=${query}`
      )
      .subscribe((res: any) => {
